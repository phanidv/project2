...
struct list_elem * get_element_to_be_eliminated() {
	...
	while (e != list_end(&frame_table)) {
		...
		if (!fte->supplementary_page_entry_->is_page_pinned) {
			...
			if (pagedir_is_accessed(fte->pagedir,
					fte->supplementary_page_entry_->user_virtual_address)) {

				pagedir_set_accessed(fte->pagedir,
						fte->supplementary_page_entry_->user_virtual_address,
						false);
			} else {
...
void eliminate_frame(struct list_elem *e) {
	...
	// If frame is dirty
	if (pagedir_is_dirty(fte->pagedir,
			fte->supplementary_page_entry_->user_virtual_address)) {
	...
	pagedir_clear_page(fte->pagedir,
			fte->supplementary_page_entry_->user_virtual_address);
	free(fte);
}
...
void swap_or_flush(struct frame_table_entry *fte) {
	...
	if (fte->supplementary_page_entry_->table_entry_type == MEM_MAPPED_PAGE) {

		flush_mmap(fte);
	} else {

		// Swap frame.
		fte->supplementary_page_entry_->swap_index = put_frame_in_swap(
				fte->frame);
		fte->supplementary_page_entry_->table_entry_type = SWAPPED_PAGE;
	}
}
...
void flush_mmap(struct frame_table_entry *fte) {
	...
	file_write_at(fte->supplementary_page_entry_->required_file, fte->frame,
			fte->supplementary_page_entry_->read_bytes,
